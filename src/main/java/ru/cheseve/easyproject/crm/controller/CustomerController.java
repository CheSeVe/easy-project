...
import ru.cheseve.easyproject.crm.enums.CustomerSort;
import ru.cheseve.easyproject.crm.service.CustomerService;
...
public class CustomerController {
    ...
    @Operation(
            ...
            description = "Возвращает клиента со списком его заказов"
    )
    @ApiResponses({
    ...
    @GetMapping("/{id}")
    public ResponseEntity<CustomerWithOrdersResponse> getCustomer(@PathVariable Long id) {
    ...
    public ResponseEntity<PageResponse<CustomerResponse>> getAllCustomers(
        ...
        log.info("GET /api/customers/, page={}, size={}, sort={}, filtered={}",
                page,
                size,
                sort,
                !filter.isEmpty());
    ...
    @Operation(
            ...
            description = "Добавляет клиента и возвращает его данные"
    )
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Клиент создан"),
            @ApiResponse(responseCode = "400", description = "Ошибка во входных данных",
                    content = @Content(schema = @Schema(implementation = ExceptionResponse.class))),
            @ApiResponse(responseCode = "409", description = "Клиент с таким email или номером телефона уже существует",
                    content = @Content(schema = @Schema(implementation = ExceptionResponse.class)))
    })
    @PostMapping
    ...
    @Operation(
            summary = "Полностью заменить существующего клиента",
            description = "Заменяет существующего клиента и возвращает его данные"
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Клиент заменен"),
            @ApiResponse(responseCode = "404", description = "Клиент не найден",
                    content = @Content(schema = @Schema(implementation = ExceptionResponse.class))),
            @ApiResponse(responseCode = "400", description = "Ошибка во входных данных",
                    content = @Content(schema = @Schema(implementation = ExceptionResponse.class))),
    ...
    @PutMapping("/{id}")
    public ResponseEntity<CustomerResponse> putCustomer(@PathVariable Long id,
    ...
    @Operation(
            summary = "Частично заменить данные у существующего клиента",
            description = "Заменяет указанные в запросе поля у существующего клиента и возвращает его данные"
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Данные у клиента заменены"),
            @ApiResponse(responseCode = "404", description = "Клиент не найден",
                    content = @Content(schema = @Schema(implementation = ExceptionResponse.class))),
            @ApiResponse(responseCode = "400", description = "Ошибка во входных данных",
                    content = @Content(schema = @Schema(implementation = ExceptionResponse.class))),
    ...
    public ResponseEntity<CustomerResponse> patchCustomer(@PathVariable Long id,
                                                          @RequestBody @Valid CustomerPatchRequest requestDTO) {
        log.info("PATCH /api/customers/{}", id);
